...
export default function RecruiterDashboard() {
  ...
  const handleDeleteJob = async (jobId: string) => {
    ...
    try {
      const response = await fetch('https://mcejnt9v3g.execute-api.us-east-1.amazonaws.com/Dev', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
            ...
            {jobs.map((job, index) => (
              <div key={index} className="border rounded-lg p-4">
                <div className="flex justify-between">
                  ...
                  <button className="text-[#ef4444]" onClick={() => openDeleteConfirmation(job.ID)}>
                    <Trash2 size={16} />
                  </button>
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={closeDeleteConfirmation} className="text-gray-500 hover:text-gray-700">
                <X size={20} />
              </button>
            </div>
            <p className="mb-6">Are you sure you want to delete this job posting?</p>
